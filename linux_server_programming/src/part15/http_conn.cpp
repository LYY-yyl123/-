...
static void addfd(int epollfd, int fd, bool one_shot)
{
    epoll_event event;
    event.data.fd = fd;
    event.events = EPOLLIN | EPOLLET | EPOLLRDHUP; /*EPOLLRDHUP代表对端断开连接*/
    if (one_shot)
    {
...
void http_conn::close_conn(bool real_close)
{
    if (real_close && (m_sockfd != -1))
    {
        ...
        m_sockfd = -1;
        m_user_count--; /*关闭一个连接，将客户总量减一（为啥不不关闭socket）*/
    }
}
...
bool http_conn::read()
{
    ...
    while (true)
    {
        ...
        if (bytes_read == -1)
        {
            if (errno == EAGAIN || errno == EWOULDBLOCK)/*对文件描述符进行非阻塞操作，无数据可读errno为EAGAIN或者EWOULDBLOCK*/
            {
                break;
            }
            return false;
...
/*我们没有真正解析HTTP请求的消息体，知识判断它是否被完整地读入了*/
...
bool http_conn::write()
{
    ...
    int bytes_to_send = http_conn::m_write_idx;
    if (bytes_to_send == 0)
    {
        modfd(http_conn::m_epollfd, m_sockfd, EPOLLIN);
        init();
        return true;
    }
    ...
    while (1)
    {
        ...
        if (temp <= -1)
        {
            /*如果一个TCP写缓冲没有空间，则等待下一轮EPOLLINOUT事件。虽然在此期间，服务器无法立即接收到同一客户下的下一个请求，但这可以保证连接的完整性*/
            if (errno == EAGAIN)
            {
                modfd(http_conn::m_epollfd, m_sockfd, EPOLLOUT);
                return true;
            }
            unmap();
            return false;

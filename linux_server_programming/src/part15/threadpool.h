...
threadpool<T>::threadpool(int thread_number, int max_requests) : m_thread_number(thread_number), m_max_requests(max_requests), m_stop(false), m_threads(NULL)
{
    ...
    for (int i = 0; i < thread_number; ++i)
    {
        ...
        if (pthread_create(m_threads + i, NULL, worker, this) != 0) /*pthread_create第一个参数为指向线程对象的指针,第三个参数必须指向一个静态函数*/
        {
            delete[] m_threads;
            throw std::exception();
        }
        if (pthread_detach(m_threads[i])) /*pthread_detach调用成功返回0*/
        {
            delete[] m_threads;
            throw std::exception();
        }
